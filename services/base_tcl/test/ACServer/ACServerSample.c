...
const char* deviceManufactureName = "DANBY";
const char* deviceProductName = "Dehumidifier-Sim";
...
static const char* DEFAULT_APP_NAMES[] = { "Danby_Dehumidifier_Sim" };
static const char* DEFAULT_DESCRIPTIONS[] = { "Dehumidifier Simulator" };
static const char* DEFAULT_MANUFACTURERS[] = { "DANBY" };
static const char* DEFAULT_DEVICE_MODELS[] = { "0.0.1" };
static const char* DEFAULT_DATE_OF_MANUFACTURES[] = { "2017-08-09" };
static const char* DEFAULT_SOFTWARE_VERSIONS[] = { "0.0.1" };
static const char* DEFAULT_HARDWARE_VERSIONS[] = { "0.0.1" };
static const char* DEFAULT_SUPPORT_URLS[] = { "http://www.danby.com" };
...
int AJ_Main(void)
{
    ...
    //Added by Peter
    //REGISTER SERVICE INTERFACES, SIGNALS, METHODS AND PROPERTIES
    ControlPanelComponents_RegisterAppObjects();   
    //End added by Peter
	...
	//Added by Peter//
	      switch (msg.msgId) {
		case ACTIONS_SET_POWER_TO_ON:
                case AIRPURIFIER_CONTROLLEE_METHOD_POWER_ON:
                    status = AppHandle_PowerON_AirPurifier();
                    break;

		case ACTIONS_SET_POWER_TO_OFF:
                case AIRPURIFIER_CONTROLLEE_METHOD_POWER_OFF:
                    status = AppHandle_PowerOFF_AirPurifier();
                    break;

                case AIRPURIFIER_CONTROLLEE_METHOD_RESET:
                    status = AppHandle_ResetAirPurifier();
                    break;

            	case AIRPURIFIER_CONTROLLEE_GET_PROPERTY_VALUE:
		    status = AJ_BusPropGet(&msg, GetPropertyValue, NULL);
                    break;

            	case AIRPURIFIER_CONTROLLEE_SET_PROPERTY_VALUE:
		    status = AJ_BusPropSet(&msg, SetPropertyValue, NULL);
                    break;
                }
